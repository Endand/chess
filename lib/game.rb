...
class Game
  ...
  def play_round
    ...
    return unless checkmate || stalemate
  ...
  def checkmate?(color)
    ...
    return false unless enemy_king_path.empty?
  ...
  def promote?(old_pos, new_pos, color)
    ...
    return false unless PAWN.include?(piece)
  ...
  def get_move(player)
    ...
    until king_safe?(old_pos, new_pos, player.color)
      ...
      king_in_check_msg unless king_safe?(old_pos, new_pos, player.color)
    end
    [old_pos, new_pos]
  end
  ...
  def get_old_input(player)
    ...
    until can_select_old?(input, player.color)
      not_your_color_msg unless match_color?(input, player.color)
      cannot_move_msg
      input = player.choose_old
    end
    input
  ...
  def can_select_old?(input, color)
    ...
    return false unless same_color?(piece, color)
  ...
  def get_new_input(player, from)
    ...
    loop do
      ...
      break if can_select_new?(path, input)
  ...
  def get_path(from, color)
    ...
    reject_blocked(path, color, from)
  end
  ...
  def pawn_path(coord, color)
    ...
    row, col = coord
    if color == 'white'
      path += add_to_path(coord, -2, 0) if row == 6
      path += add_to_path(coord, -1, 0)
      dirs = [[-1, -1], [-1, 1]]
    elsif color == 'black'
      path += add_to_path(coord, 2, 0) if row == 1
      path += add_to_path(coord, 1, 0)
      dirs = [[1, 1], [1, -1]]
    end
    dirs.each do |dr, dc|
  ...
  def remove_blocked_path(path, _color, coord)
    piece_type = coord_to_piece_type(coord)
    case piece_type
    when 'rook'
  ...
  def get_blocked_path(dirs, coord, path)
    dirs.each do |dr, dc|
      ...
      while in_bound?([r + dr, c + dc])
        ...
        path.delete([r, c]) if blocked && path.include?([r, c])

        blocked = true if @board.show_coord(r, c) != ' '
      end
    end
    path
  end
  ...
  def promo_get_response
    choices = %w[queen rook bishop knight]
